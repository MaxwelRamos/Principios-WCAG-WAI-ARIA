function ListaCompleta() {
	...
	listaCompleta.map(function (lista) {
	  conteudo += `<br /><li class="praticoTresLista__item" aria-live="polite">${lista}</li>`;
	});
  ...
  function ListaQuantidade() {
	...
	let conteudo = `<br /><li class="praticoTresLista__item praticoTresLista__item--quantidade" aria-live="polite">${listaCompleta.length}</li>`;
	$('.praticoTresLista').html(conteudo);
  }
...
function FullScreen(){
	$('.figura').html(`<img id="figura__fundo" alt="Tela apresentacao do sistema" src="img/fundo1024.bmp" />`);
